...
FEAT2IDX = {x: y for y, x in zip(FEATS, range(len(FEATS)))}
logging.info(f'使用的EEG通道: {FEATS}')

# 3. 预计算滤波器参数
def get_lowpass_coeffs(cutoff_freq=40, sampling_rate=200, order=4):
    nyquist = 0.5 * sampling_rate
    normal_cutoff = cutoff_freq / nyquist
    b, a = butter(order, normal_cutoff, btype='low', analog=False)
    return b, a
...
# 获取滤波器系数
b, a = get_lowpass_coeffs(cutoff_freq=40, sampling_rate=200, order=4)

# 4. 应用低通滤波器并下采样
def butter_lowpass_filter_downsample(data, b, a, downsample_factor=5):
    # 使用filtfilt以避免相位失真
...
# 5. 数据预处理函数：滤波、下采样、归一化
def preprocess_transform(segment, t_axis, downsample_factor=5, fixed_length=2000, snippet_id=None):
    """
    对EEG数据应用低通滤波、下采样并进行通道归一化。
    """
    # 处理近似常数通道
    variances = np.var(segment, axis=0)
    constant_channels = np.where(variances < 1e-6)[0]  # 使用小阈值检测近似常数通道
    ...
    if len(constant_channels) > 0:
        logging.warning(f"Snippet {snippet_id}: Constant channels detected: {constant_channels}. Adding small noise.")
        for ch in constant_channels:
            # 增加较小的随机噪声以防止滤波器引入 NaNs
            segment[:, ch] += np.random.normal(0, 1e-3, size=segment.shape[0])

    # 应用低通滤波器并下采样
    try:
        filtered_segment = butter_lowpass_filter_downsample(
            segment,
            b,
            a,
        ...
        logging.error(f"Snippet {snippet_id}: Error during filtering and downsampling: {e}")
        raise e
    ...
    # 通道归一化
    mean = filtered_segment.mean(axis=0)
    std = filtered_segment.std(axis=0)
    std[std < 1e-6] = 1.0  # 防止除以零
    normalized_segment = (filtered_segment - mean) / std

    # 最终检查
    if np.isnan(normalized_segment).any():
        logging.error(f"Snippet {snippet_id}: Normalized segment contains NaNs!")
        raise AssertionError("Normalized segment contains NaNs!")
    if np.isnan(t_norm).any():
        logging.error(f"Snippet {snippet_id}: Normalized time axis contains NaNs!")
        raise AssertionError("Normalized time axis contains NaNs!")

    # 确保固定长度
    if normalized_segment.shape[0] != fixed_length:
        logging.warning(f"Snippet {snippet_id}: Expected length {fixed_length}, but got {normalized_segment.shape[0]}. Adjusting.")
        if normalized_segment.shape[0] < fixed_length:
            # 填充
            pad_length = fixed_length - normalized_segment.shape[0]
            normalized_segment = np.pad(normalized_segment, ((0, pad_length), (0,0)), 'constant')
            t_norm = np.pad(t_norm, (0, pad_length), 'constant')
        else:
            # 截断
            normalized_segment = normalized_segment[:fixed_length, :]
            t_norm = t_norm[:fixed_length]

    return normalized_segment, t_norm
...
class SnippetDataset(Dataset):
    ...
    def __getitem__(self, idx):
        snippet_dict = torch.load(self.snippet_paths[idx])
        X = snippet_dict["X"]  # shape (2000, 19) after 5x下采样
        t = snippet_dict["t"]  # shape (2000,)
        y = snippet_dict["y"]  # scalar
        return (X, t, y)
...
def extract_eeg_segment_with_times(
    ...
    """
    从Parquet文件中提取指定偏移点后的EEG片段（duration_seconds秒），
    并构建对应的时间轴t（长度 = duration_seconds * sample_rate）。
    """
    start_idx = int(offset_seconds * sample_rate)
    end_idx = start_idx + int(duration_seconds * sample_rate)
    ...
    # 截取EEG片段
    eeg = df_eeg.iloc[start_idx:end_idx].to_numpy(dtype=np.float32)  # shape ~ (10000, 19)
...
def generate_5fold_splits_chunked(
    ...
    """
    1. 从train.csv读取元数据，每行 -> 提取50秒EEG + 时间轴t +标签（投票最大） -> 保存到单独.pt
    2. 使用StratifiedKFold做5折，把snippet路径 +标签分成(train_idx, test_idx)，
       再对test_idx做一次二分，得到val_idx + test_idx2。
    3. 对训练集：用SnippetDataset + DataLoader实现懒加载并存入final dict；
       对验证集、测试集：一次性加载 (X, t, y) 并拼为 (N, T, C)/(N, T)/(N, )。
    4. 最后在output_path下保存split_hms_{fold}.pt。
    """
    # 读取CSV
    train_csv_path = os.path.join(data_path, "train.csv")
    if not os.path.isfile(train_csv_path):
    ...
    # 检查train.csv里的例子
    logging.info(f"Train metadata shape: {train_metadata.shape}")
    logging.info(f"Train metadata head:\n{train_metadata.head(10)}")
    ...
    available_eeg_ids = set(os.listdir(train_eegs_path))
    available_eeg_ids = {eid.split('.parquet')[0] for eid in available_eeg_ids if eid.endswith('.parquet')}
    logging.info(f"Available EEG IDs sample: {list(available_eeg_ids)[:10]}")

    # 筛选出train_metadata的eeg_id列存在于数据文件夹中的eeg_ids
    train_metadata['eeg_id_str'] = train_metadata['eeg_id'].astype(str)
    train_metadata = train_metadata[train_metadata['eeg_id_str'].isin(available_eeg_ids)]
    logging.info(f"Filtered train metadata shape: {train_metadata.shape}")
    ...
    # 创建临时目录：存放每个样本 snippet_{i}.pt
    temp_snippets_dir = os.path.join(output_path, "temp_snippets_19ch")  # 新目录以区分
    os.makedirs(temp_snippets_dir, exist_ok=True)

    snippet_paths = []  # 记录snippet文件路径
    snippet_labels = []  # 记录每个snippet的标签

    # 遍历train_metadata，每行生成一个snippet .pt
    for i, row in tqdm(train_metadata.iterrows(), total=len(train_metadata), desc="Extracting snippets"):
        eeg_id = row["eeg_id"]
        offset_sec = row["eeg_label_offset_seconds"]
        file_path = os.path.join(train_eegs_path, f"{eeg_id}.parquet")
        ...
        try:
            filtered_segment, t_norm = preprocess_transform(
                segment,
                t_axis,
                downsample_factor=5,  # 5倍下采样
                fixed_length=2000,    # 下采样后长度应为10000 / 5 = 2000
                snippet_id=i
            )
        except AssertionError as e:
            logging.error(f"Snippet {i}: {e}. Skipping this snippet.")
        ...
        # 可视化第一个snippet的EEG信号（可选）
        if i == 0:
            plt.figure(figsize=(24, 10))
            offset_plot = 5  # 偏移量防止重叠
            for c in range(filtered_segment.shape[1]):
                plt.plot(range(filtered_segment.shape[0]), filtered_segment[:, c] + c * offset_plot, label=f'Channel {c + 1} ({FEATS[c]})')
            plt.title(f'EEG Signal for Class {label_idx}')
            plt.legend()
            plt.savefig('eeg_signal_sample_19ch.png')
            plt.close()

        # 将(X, t, y)存到snippet_i.pt
        snippet_dict = {
            "X": torch.tensor(filtered_segment.copy(), dtype=torch.float32),  # shape (2000, 19)
            "t": torch.tensor(t_norm.copy(), dtype=torch.float32),             # shape (2000,)
            "y": torch.tensor(label_idx, dtype=torch.long)                    # scalar
        }
        snippet_file = os.path.join(temp_snippets_dir, f"snippet_{i}.pt")
        torch.save(snippet_dict, snippet_file)
    ...
    for train_idx, test_idx in skf.split(snippet_paths, snippet_labels):
        ...
        val_X = torch.stack(val_X_list, dim=0)  # shape (N_val, 2000, 19)
        val_t = torch.stack(val_t_list, dim=0)  # shape (N_val, 2000)
        val_y = torch.tensor(val_y_list, dtype=torch.long)  # shape (N_val,)
        ...
        test_X = torch.stack(test_X_list, dim=0)  # shape (N_test, 2000, 19)
        test_t = torch.stack(test_t_list, dim=0)  # shape (N_test, 2000)
        test_y = torch.tensor(test_y_list, dtype=torch.long)  # shape (N_test,)

        # 封装成字典并保存
        fold_dataset = {
            "train_loader": train_loader,        # PyTorch DataLoader
            "val": (val_X, val_t, val_y),        # (N_val, 2000, 19)/(N_val, 2000)/(N_val,)
            "test": (test_X, test_t, test_y)     # (N_test, 2000, 19)/(N_test, 2000)/(N_test,)
        }

        # 保存到新的目录，避免覆盖
        split_output_path = os.path.join(output_path, f"fold_{fold_id}")
        os.makedirs(split_output_path, exist_ok=True)
        fold_file = os.path.join(split_output_path, f"split_hms_fold_{fold_id}.pt")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Generate 5-fold splits for EEG data with modifications.")
    parser.add_argument('--data_path', default="/root/autodl-tmp/time/datasets/hms", type=str, required=False,
                        help='Path to the data directory containing train.csv and train_eegs/')
    parser.add_argument('--output_path', default="/root/autodl-tmp/time/datasets/hmstrain2",type=str, required=False,
                        help='Path to save the split files and preprocessed snippets')
    parser.add_argument('--n_splits', type=int, default=5, help='Number of folds for cross-validation')
    args = parser.parse_args()
